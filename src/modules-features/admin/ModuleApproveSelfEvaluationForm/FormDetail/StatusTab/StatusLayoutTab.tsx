...
export default function StatusLayoutTab() {
  const [content, setContent] = useState<string>(mocDataContent);

  const handleAddEvidence = (code: string) => {
    setContent((prev) => prev + ` <a href="#">${code}</a>`);
  };
    ...
    <MyFlexColumn gap={16}>
      ...
      <Grid>
        ...
        <Grid.Col span={{ md: 12, xl: 5 }}>
          <EvidenceListTable onAddEvidence={handleAddEvidence} />
        </Grid.Col>
      </Grid>
    </MyFlexColumn>
  );
}
const mocDataContent = `
